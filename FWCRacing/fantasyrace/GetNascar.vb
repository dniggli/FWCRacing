...
Module GetNascar
        ...
        For Each s As String In splt
            ...
            If (s.Contains("Running") Or s.Contains("Accident") Or s.Contains("Vibration") Or s.Contains("Engine") Or s.Contains("Rear End")) = True Then
                Try
                    ...
                    Dim comm As New MySqlCommand("insert into fantasy_race.nascarresults(position,driver,laps,pollgrid,circut,running,stagewin) Values ('" & position & "','" & driver & "','" & laps & "','" & pollgrid & "','" & circut & "','" & running & "','" & Stage1 & "')", New MySqlConnection("server=" & Form1.server & ";uid=root;pwd=vvo084;"))
    ...
    '    post.Url = "http://www.race-database.com/results/results.php?year=" & testyears & "&race=" & race & "&series_id=2"
    ...
    '                Dim comm As New MySqlCommand("insert into fantasy_race.nascarresults(position,driver,laps,pollgrid,circut,running) Values ('" & list(list0) & "','" & list(list2) & "','" & list(list11) & "','" & list(list1) & "','" & racelist(RaceNumbers) & "','" & list(list12) & "')", New MySqlConnection("server=" & Form1.server & ";uid=root;pwd=vvo084;"))
