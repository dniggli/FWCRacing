...
Public Class Form1
    ...
    Sub populatenascarreults()
        Dim das As New MySqlDataAdapter("select nascarb.key,NasCarBDriver,position,pollgrid,circut,running,laps,stagewin from fantasy_race.nascarb,fantasy_race.nascarresults where nascarb.NasCarBDriver = nascarresults.driver order by laps desc", "server=" & Form1.server & ";uid=root;pwd=vvo084;")
        Dim ts As New DataTable
        das.Fill(ts)


        Dim da As New MySqlDataAdapter("select nascara.key,NasCarADriver,position,pollgrid,circut,running,laps,stagewin from fantasy_race.nascara,fantasy_race.nascarresults where nascara.NasCarADriver = nascarresults.driver order by laps desc", "server=" & Form1.server & ";uid=root;pwd=vvo084;")
        Dim t As New DataTable
        da.Fill(t)
            ...
            For Each ctl As Control In GroupBox1.Controls
                ...
                If TypeOf ctl Is TextBox Then
                    ...
                    If ctl.Tag = key Then
                        ...
                        If Not status Like "Running" Then pos = pos + "D"
    ...
    Sub StandingsCalc()
        ...
        For Each dr As DataRow In t.Rows
            ...
            Dim comm As New MySqlCommand("insert into fantasy_race.leaderboard(Team,Behind,Wins,Poles,TopTen,TopFive,StageWins,LapsLead,DNF,Points) values('" & teams & "', '" & pointbehind & "', '" & wins & "', '" & poles & "', '" & ten & "', '" & five & "','" & swins & "', '" & laps & "', '" & dnfs & "', '" & totals & "')", New MySqlConnection("server=" & Form1.server & ";uid=root;pwd=vvo084;"))
        ...
        For Each addt As DataRow In addp.Rows
            ...
            Dim comm As New MySqlCommand("insert into fantasy_race.leaderboard(Team,Behind,Wins,Poles,TopTen,TopFive,StageWins,LapsLead,DNF,Points,Place) values('" & teams & "', '" & pointbehind & "', '" & wins & "', '" & poles & "', '" & ten & "', '" & five & "','" & swins & "', '" & laps & "', '" & dnfs & "', '" & totals & "', '" & place & "')", New MySqlConnection("server=" & Form1.server & ";uid=root;pwd=vvo084;"))
            ...
            place = place + "1"
        Next
        Dim myda As New MySqlDataAdapter("select Place as 'Position',Team,Points,Behind,Wins,Poles,TopTen,TopFive,StageWins,LapsLead,DNF from fantasy_race.leaderboard order by Points desc", "server=" & Form1.server & ";uid=root;pwd=vvo084;")
        ...
        For Each drs As DataRow In lbt.Rows
            ...
            For Each dc As DataColumn In lbt.Columns
                ...
                ElseIf I = "5" Then
                    ROW2 &= "<td bgcolor = 'yellow'><font size='1' color ='red'><b>*****PAYOUT TO TOP 5*****<br></font></td>"
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        If response = MsgBoxResult.Yes Then
            ...
            Dim comm8 As New MySqlCommand("insert into fantasy_race.week(lastweek) values('0')", New MySqlConnection("server=" & Form1.server & ";uid=root;pwd=vvo084;"))
    ...
    Private Sub Buttonresetweeks_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Buttonresetweeks.Click
        ...
        For Each pair In dict
            ...
            Dim comm1 As New MySqlCommand("Truncate fantasy_race." & pair.Key & ";", New MySqlConnection("server=" & Form1.server & ";uid=root;pwd=vvo084;"))
            ...
            If pair.Key = "week" Then
                ...
                Dim comm2 As New MySqlCommand("insert into fantasy_race." & pair.Key & "(lastweek)values(" & pair.Value & ");", New MySqlConnection("server=" & Form1.server & ";uid=root;pwd=vvo084;"))

                comm2.Connection.Open()
                comm2.ExecuteNonQuery()
                comm2.Connection.Close()
            Else
                Dim comm2 As New MySqlCommand("insert into fantasy_race." & pair.Key & "(racenumber)values(" & pair.Value & ");", New MySqlConnection("server=" & Form1.server & ";uid=root;pwd=vvo084;"))
